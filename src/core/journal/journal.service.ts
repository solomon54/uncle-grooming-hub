...
export class JournalService {
  ...
  /** Count unsynced events (for sync engine status) */
  public async countUnsynced(): Promise<number> {
    if (!this.collection) return 0;
    const docs = await this.collection
      .find({ selector: { is_synced: false } })
  ...
  /** Get next aggregate version for an aggregate (used by action creators) */
  public async getNextAggregateVersion(aggregateId: string): Promise<number> {
    if (!this.collection) return 1;
    const latest = await this.collection
      .findOne({
  ...
  /**
   * Ingest a cloud-authority event (EVENT 08, 19, etc.) received via sync pull.
   * Bypasses CLOUD_AUTHORITY guard — these events ARE from the cloud.
   * Still enforces VERSION_CONFLICT and DUPLICATE_EVENT guards.
   */
  public async ingestCloudEvent(event: AllEvents): Promise<CommitResult> {
    if (!this.collection) {
      throw new Error("[JournalService] Not initialized.");
    }
    ...
    const hlc = clockService.receive(event.metadata.hlc_timestamp);
    ...
    await this.collection.insert({
      ...
      hlc,
      is_synced:         true, // Cloud events are already synced
    });
